...
async function migrate() {
    ...
    try { await db.run('ALTER TABLE timeslots ADD COLUMN location TEXT'); } catch (e) { }
    try { await db.run('ALTER TABLE timeslots ADD COLUMN price_youth INTEGER'); } catch (e) { }
    try { await db.run('ALTER TABLE timeslots ADD COLUMN price_child INTEGER'); } catch (e) { }
    ...
    console.log('Migration and data update complete!');
    const slots = await db.all('SELECT * FROM timeslots');
    console.log('Final slots in DB:', slots);
    await db.close();
}
migrate();
